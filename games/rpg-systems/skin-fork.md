...
É permitido então nadar a qualquer um como modo livre da natação, mas existe risco de vida (mais sobre saúde e fluxo adiante).
...
No exemplo indicado no modo livre, o jogador que tentar nadar vai tomar -1 de saúde, o que pode ser fatal (se ele ja estiver em pessimo,
qualquer resultado negativo no fluxo - v seção especifica - vai mata-lo).
...
e as cartas podem solicitar valores nos dados do pool (por ex, "com a parte que tiver um 5 no pool acontece ..."), primais, estados de saude,
...
As cartas tem muitas vezes niveis (variando de -2 a +2, que acompanham a escala geral do jogo: pessimo (-2), ruim, medio, bom e excelente(+2),
que podem ajudar a interpretar o que deve ser feito.
...
### Natação
    ...
    Natação
     Fortuna ou Anima: Pode subir um nivel (transitorio) mesmo em uso geral. Cumulativo.
     Arte: Cada cubo (permanente) aumenta o nivel em 1 (pode passar de 2)
     Estamina: Cada cubo (permanente) aumenta o nivel em 1 (pode passar de 2 )
     
     Uso:
     pessimo: -1 de saude, morte é possivel (fluxo), precisa ser salvo
                Não encerra a interação se nao for salvo 
...
### Montar Animais
    ...
    Montar Animais
     Fortuna: Pode subir um nivel (transitorio) mesmo em uso geral.
     Arte: Cada cubo (permanente) aumenta o nivel em 1 (pode passar de 2)
     Balanço: Cada cubo (permanente) aumenta o nivel em 1 (pode passar de 2 )
     Mana: Pode subir um nivel (transitorio) mesmo em uso geral, somente para criaturas magicas
        Cada cubo (permanente) aumenta o nivel em 1 (pode passar de 2), mas deve ser afixado somente
        em ficha de criatura magica.

     Esta capacidade acompanha Empatia com Animais.
     
     Uso:
     - Escolha um animal, esta ficha deve ser usada somente para a escolha.
     - Para andar em qualquer outro animal, a primeira tentativa é uso livre. Em caso de sucesso, replica-se essa ficha para o outro
     - Em todas as fichas de montar animais, o nivel incial será um abaixo do melhor nivel em uma delas (por ex, cavalgar excelente
       = montar bom em todas as outras fichas)
     
     pessimo: O animal não permite a montagem. Se for forçado, tentará derrubar.
                Qualquer resultado no fluxo é possivel.
     
     ruim: É possivel montar, mas em qualquer situaçao mais delicada o animal
                    vai para onde quiser. 
     ruim (geral): Alem do acima, existe risco de queda e -1 de saude, mas morte somente em -2.
     
     médio: É possivel montar, mas em situaçao critica o animal
                    vai para onde quiser. Nivel de sucesso, no entanto (=0)
     
     bom: O animal está sempre sobre controle, sucesso = 1
     
     excelente: O animal está sempre sobre controle, sucesso = 2
...
### Empatia com Animais
    ...
    Empatia com Animais
     ...
     ruim: O animal não tem nenhuma afinidade, e atacará +1 se for selvagem, como aviso somente (0) se domesticado. 
     ruim (geral): idem
     
     médio: Animais selvagens atacam como aviso somente (0). Domesticos não atacam, mas nao tem afinidade.
     
     bom: Animais domesticos tem afinidade e seguem / obedecem, mas com hesitação se for contra a natureza deles.
           Animais selvagens não atacam, mas nao tem mais afinidade que isso, e atacarão se provocados.
     
     excelente: Animais domesticos tem afinidade e seguem / obedecem cegamente, chegando a gostar muito de voce.
                Animais selvagens tem afinidade e seguem / obedecem, mas com hesitação se for contra a natureza deles.
                Hesitam em atacar.
